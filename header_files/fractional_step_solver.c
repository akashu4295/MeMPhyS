...
double fractional_step_explicit_vectorised(PointStructure* myPointStruct, FieldVariables* field){   
    ...
    for (int i = 0; i < myPointStruct[0].num_nodes; i++){
        steady_state_error += (field[0].u[i] - field[0].u_old[i])*(field[0].u[i] - field[0].u_old[i]) + (field[0].v[i] - field[0].v_old[i])*(field[0].v[i] - field[0].v_old[i]) + (field[0].w[i] - field[0].w_old[i])*(field[0].w[i] - field[0].w_old[i]);
    }
