        ...
        for (int i = 0; i< mypointstruct->num_nodes; i++){
            if(!mypointstruct->boundary_tag[i]){
                ...
                for (int j = 0; j < mypointstruct->num_cloud_points; j++){
                    field->res[i] = field->res[i] - mypointstruct->lap_Poison[k]*field->pprime[mypointstruct->cloud_index[k]];
                    k += 1;
                }
            }
            else{
...
void calculate_intermediate_velocity_implicit_vectorised_2d(PointStructure* myPointStruct, FieldVariables* field){
    ...
    double denom = 0;
    double temp1, temp2, temp3, temp4;
    double advection_x, advection_y, advection, diffusion, unsteady;
...
void prolongate_corrections_vectorised_2d(PointStructure* mypointStruct_f, PointStructure* mypointStruct_c, FieldVariables* field_f, FieldVariables* field_c){
    int n = mypointStruct_c->num_cloud_points;
    # pragma acc parallel loop present(field_f, field_c, mypointStruct_f, mypointStruct_c)
    for (int i = 0; i < mypointStruct_f->num_nodes; i++){
        if (mypointStruct_f->boundary_tag[i]==false){
...
void update_boundary_pressure_vectorised_2d(PointStructure* myPointStruct, FieldVariables* field){
    ...
    if(myPointStruct->boundary_tag[i]){
        ...
        field->dpdn[i] = field->rho * (temp1*myPointStruct->x_normal[i] + temp2*myPointStruct->y_normal[i]);

        sumx = 0; sumy = 0; Ap = 0.0;
        # pragma acc loop reduction(+:sumx, sumy, Ap)
        k = i*n;
        for (int j = 1; j < myPointStruct->num_cloud_points; j++){
            k += 1;
