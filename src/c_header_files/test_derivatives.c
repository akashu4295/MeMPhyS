...
void calculate_errors_2d(PointStructure* myPointStruct, double* f, double* fx, double* fy, double* lapf, 
                          double* F, double* Fx, double* Fy, double* lapF){
    double l2_error_f    = l2_norm_relative(myPointStruct, F, f, myPointStruct->num_nodes);
    double l2_error_fx   = l2_norm_relative(myPointStruct, Fx, fx, myPointStruct->num_nodes);
    double l2_error_fy   = l2_norm_relative(myPointStruct, Fy, fy, myPointStruct->num_nodes);
    double l2_error_lapf = l2_norm_relative(myPointStruct, lapF, lapf, myPointStruct->num_nodes);
    printf("Relative L2 error (interior points only):\n");
    printf("  f    : %lf%%\n", l2_error_f    * 100);
    printf("  fx   : %lf%%\n", l2_error_fx   * 100);
    printf("  fy   : %lf%%\n", l2_error_fy   * 100);
    printf("  lapf : %lf%%\n", l2_error_lapf * 100);
...
void calculate_errors_3d(PointStructure* myPointStruct, double* f, double* fx, double* fy, double* fz, double* lapf, 
                          double* F, double* Fx, double* Fy, double* Fz, double* lapF){
    double l2_error_f    = l2_norm_relative(myPointStruct, F, f, myPointStruct->num_nodes);
    double l2_error_fx   = l2_norm_relative(myPointStruct, Fx, fx, myPointStruct->num_nodes);
    double l2_error_fy   = l2_norm_relative(myPointStruct, Fy, fy, myPointStruct->num_nodes);
    double l2_error_fz   = l2_norm_relative(myPointStruct, Fz, fz, myPointStruct->num_nodes);
    double l2_error_lapf = l2_norm_relative(myPointStruct, lapF, lapf, myPointStruct->num_nodes);
    printf("Relative L2 error (interior points only):\n");
    printf("  f    : %lf%%\n", l2_error_f    * 100);
    printf("  fx   : %lf%%\n", l2_error_fx   * 100);
...
double l2_norm_relative(PointStructure* myPointStruct, double *A, double *B, int n_rows_A) {
    ...
    for (int i = 0; i < n_rows_A; i++) {
        if (myPointStruct->boundary_tag[i] == 0) {
            double diff = A[i] - B[i];
            error_sq += diff * diff;
            ref_sq   += B[i] * B[i];  // B is the analytical reference
            count++;
        }
    }

    if (ref_sq == 0.0 || count == 0) {
        printf("Warning: reference norm is zero or no interior points found.\n");
        return 0.0;
    }

    return sqrt(error_sq / ref_sq);
}
